...
function enableButtons () {
	...
	btn.onclick=function(){greetPeriod();};
	
	// Update the button label now that the button is active
	btn.value="Click me for a personal greeting";

	btn = document.getElementById("submit_person");
	btn.onclick=function(){registerPerson();};
	
	// Update the button label now that the button is active
	btn.value="Register Person";

	//go through database and show all reg people
	btn = document.getElementById("all_people");
	btn.onclick=function(){showRoster()};
	
	// Update the button label now that the button is active
	btn.value="Show People Registered";
}
...
function greetPeriod(){
	var name = document.getElementById("name_field").value;
	var period = document.getElementById("pf").value;
...
function registerPerson(){
	var name = document.getElementById("id_field").value;
	var age = document.getElementById("age_field").value;

	var request = gapi.client.helloworldendpoints.storePerson({'name':name , 'age':age})
	request.execute(sayHelloCallback);
}

function showRoster(){
	
	var request = gapi.client.helloworldendpoints.displayPeople();
	request.execute(sayHelloCallback);
}

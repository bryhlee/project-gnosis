...
REQUEST_PERSON_CONTAINER = endpoints.ResourceContainer(
    name=messages.StringField(1),
...
class HelloWorldApi(remote.Service):
    ...
    @endpoints.method(REQUEST_PERSON_CONTAINER, Hello, 
      path = "storePerson", http_method="GET", name="storePerson")
    def store_person(self, request):
      greet = "Registering...\nPerson: {} \n Age: {}".format(request.name, request.age)

      '''create  and store new person'''
      p=Person(name=request.name, age=request.age)
      p.put();

      return Hello(greeting=greet)

    @endpoints.method(message_types.VoidMessage, Hello,
        path = "displayPeople", http_method='GET', name="displayPeople")
